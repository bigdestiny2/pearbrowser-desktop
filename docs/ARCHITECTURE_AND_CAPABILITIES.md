...
- Desktop automated tests: `npm test` passing, 443 tests. The PR Desktop CI
...
- Featured app bundles: Keet fresh-peer sampled with zero missing sampled blobs on the latest pass. Peercord's catalogue row remains reachable and correctly marked `standalone`, and the latest source audit confirms the desktop/window contract at upstream commit `ea260a3bfba279769acfbfe0a436140c87a0fa15`, but the 2026-06-28 full-bundle refresh is deferred: the public drive currently resolves to an empty checkout from a fresh peer and needs Peercord publisher/operator reseed or complete publisher storage before it is current-pass evidence.
