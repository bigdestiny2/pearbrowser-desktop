...
- A follow-up generated Expo iOS Debug simulator build now succeeds with signing disabled and DerivedData under `/private/tmp`; it includes the `ExpoLinking` pod, runs `[Expo] Configure project`, and copies/strips the BareKit xcframework stack. The generated Expo Release simulator path is still not release-cleared: `export:embed` emits a 7.5 MB JS bundle, then Hermes bytecode compilation stalls before producing `PearBrowser.app/main.jsbundle`. A direct `hermesc -Og` repro against the generated bundle also stalled for more than 60 seconds and was interrupted.
...
Required before native/mobile distribution: clean up the generated Expo iOS Release/Hermes bundle path if that compatibility shell remains a target, validate Android with the real upload/release keystore, run app-store-style distribution checks, and broaden simulator/emulator smoke to real devices. Local Node tests do not prove platform WebView behavior under real OS permissions.
...
- Clear remaining native mobile smoke before any app-store-style mobile announcement: production Android signing/store validation, broader real-device testing, and generated Expo iOS Release/Hermes cleanup if that compatibility shell remains a target.
...
- Native mobile is not app-store-release-cleared yet. The JS/test surface is green, the missing `ExpoLinking` dependency is fixed, generated Expo iOS Debug builds, the tracked SwiftUI iOS shell reaches `Connected`, Android native debug APK assembly plus emulator launch pass, and Android release APK/AAB plus disposable-key signing verification pass. Remaining gates are generated Expo iOS Release/Hermes cleanup if that compatibility shell remains release-targeted, production Android keystore/store validation, broader real-device validation, and app-store-style signing.
