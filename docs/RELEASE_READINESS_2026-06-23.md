...
One earlier desktop `npm test` run reported `401/402`; the immediately repeated compact full run passed `402/402`, and the current release branch now passes `432/432` after the Peercord launch-mode, peerit, catalogue-dedupe, naming-target, runtime-smoke, release-doc, release-evidence checker, live-catalogue provenance, announcement-decision fail-closed coverage, startup landing regression coverage, and in-page `hyper://` link bridge coverage landed. No code change was needed for that earlier blip.
