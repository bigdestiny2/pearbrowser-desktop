...
- Desktop source-install reproducibility is standalone via npm. **UPDATE 2026-06-29:** HiveRelay is published to npm at `0.20.2` and the desktop pins `^0.20.2`, so a standalone clone resolves it from the registry — no vendored tarballs. `scripts/check-hiverelay-layout.mjs` is warn-only for registry deps; the sibling `../../00-core/hiverelay` checkout is optional dev-only. (Superseded the earlier `vendor/hiverelay/*.tgz` approach.)
- Desktop GitHub Actions CI: `.github/workflows/desktop-ci.yml` checks out PearBrowser desktop and the mobile source contract, runs the warn-only layout check, asserts the lockfile-pinned HiveRelay versions are **published to npm**, runs `npm ci`, runs the desktop test suite, and runs the high-severity dependency audit.
...
- `scripts/check-hiverelay-layout.mjs` confirms HiveRelay installs from npm (`^0.20.2`) for standalone source installs and warns, rather than fails, when the optional sibling HiveRelay checkout is absent.
...
node scripts/check-hiverelay-layout.mjs # desktop source install guard; confirms HiveRelay installs from npm (^0.20.2), sibling checkout optional
