...
- Regenerated `backend/catalogue-seed.js` from the publisher source so the offline seed and live catalogue agree.
...
- From a different network, run `node scripts/verify-pin.js --expect <releaseLength>`.
...
- Open the Apps tab and confirm the live catalogue loads Peercord from the Hyperbee key above.
...
node scripts/publish-catalog-bee.js /Users/localllm/Projects/pear-ecosystem/03-sites/pearbrowser-publishers/catalog-source/catalog.json --storage /Users/localllm/Projects/pear-ecosystem/03-sites/pearbrowser-publishers/catalog
```
...
- Live GUI launch was not proven inside this local test pass. Automated tests prove the routing contracts; a real Pear window smoke still matters.
- Network replication can vary by relay health and NAT conditions. The catalogue publish was accepted by 5 relays, but the app release should still be checked from a separate network before public announcement.
