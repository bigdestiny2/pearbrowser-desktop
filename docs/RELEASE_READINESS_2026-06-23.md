...
- Desktop: `npm test` passed `450/450` after the native release asset resolver, native release asset checker, release story smoke tooling, evidence updates, and vendored HiveRelay source-install guard.
...
npm run check:native-release-assets -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --json # verified 16 attached native release assets: macOS app.zip, Windows exe/msix, Linux AppImages, per-artifact sidecars, checksum indexes, and manifests
