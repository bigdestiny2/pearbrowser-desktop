...
- Desktop: `npm test` passed `478/478` after guarded signing secret setup templates, release evidence final-decision blocker normalization, the public-trust operator report evidence-handoff command update, the release evidence handoff generator, Nostr signature tamper-test hardening, public-trust operator report generator, native signing secret plan generator, direct-link native install guide generator, GitHub Actions signing secret-name preflight update, dual-architecture macOS native packaging update, native release public-trust workflow mode, native release asset resolver/checker, native download verifier, Linux AppImage metadata gate, package-manager license metadata, native install snippet generator, native install smoke plan generator, package-manager manifest draft generator, public-trust readiness aggregator, public-trust macOS DMG gate, release story smoke tooling, evidence updates, and HiveRelay registry source-install guard.
...
- Desktop dependency audit: `npm audit --audit-level=high` found 0 vulnerabilities after the HiveRelay registry lockfile update.
...
- Desktop source-install reproducibility is standalone via npm. **UPDATE 2026-06-29:** HiveRelay is published to npm at `0.20.2` and the desktop pins `^0.20.2` with lockfile `0.20.2`, so a standalone clone resolves it from the registry — no vendored tarballs and no sibling checkout required. `scripts/check-hiverelay-layout.mjs` exits quietly for registry deps, fails on dependency/lockfile drift, and validates `../../00-core/hiverelay` only when all three HiveRelay deps are explicit `file:` workspace specs. (Superseded the earlier `vendor/hiverelay/*.tgz` approach.)
- Desktop GitHub Actions CI: `.github/workflows/desktop-ci.yml` checks out PearBrowser desktop and the mobile source contract, runs the quiet layout check, asserts the lockfile-pinned HiveRelay versions are **published to npm**, runs `npm ci`, runs the desktop test suite, and runs the high-severity dependency audit.
- Desktop native packaging is now wired through `.github/workflows/desktop-native-release.yml`: macOS emits ad-hoc signed `.app.zip` assets for Apple Silicon and Intel, Windows emits `.msix` plus `.exe`, Linux emits `.AppImage`, and the collector attaches checksums plus manifests per platform/architecture. The workflow now separates `release_mode=package-proof` from `release_mode=public-trust`: manual package-proof runs can refresh ad-hoc/unsigned assets, while public-trust, release-published, and tag-triggered runs fail closed on missing macOS Developer ID/notary or Windows signing credentials, create notarized macOS DMGs before artifact collection, require macOS DMGs in the post-upload public-trust asset check, and require the GitHub release to be published. The Linux path now checks AppImage desktop integration metadata before artifact collection: committed Linux icon and AppStream metainfo are verified locally, and Linux builds must produce an AppDir with `PearBrowser.desktop`, `icon.png`, and `usr/share/metainfo/io.github.bigdestiny2.pearbrowser.metainfo.xml`. The root package and AppStream metadata now carry the explicit `Apache-2.0 AND MIT` SPDX expression, so package-manager drafts no longer default the WinGet license to `Unknown`. The `v0.5.0` GitHub release asset backfill completed successfully again on 2026-06-28 in Desktop Native Release run `28321639492` from `main` with `source_ref=main` and `release_mode=package-proof` after the macOS Intel release-target merge; macOS Apple Silicon, macOS Intel, Windows, Linux, and attach jobs passed, and the attach job refreshed 20 release assets. `npm run check:native-release-assets -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --json` now verifies the live GitHub attachment set and passed with 20 assets, zero warnings, and zero errors. `npm run verify:native-downloads -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --all --json` downloaded the recommended macOS arm64, macOS x64, Windows x64, and Linux x64 packages and verified all four against their attached `.sha256` sidecars. `npm run check:linux-appimage-metadata -- --json` verifies the Linux metadata source, and the native workflow runs it with `--build-dir appling/build` on Linux. `npm run resolve:native-release -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --platform macos --arch arm64` and `--platform macos --arch x64` resolve the current macOS packages and checksum sidecars for user-facing install guidance. `npm run -s generate:native-install-snippet -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop` emits release-note/install-page Markdown from the same resolver choices, and `npm run -s generate:native-install-guide -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop` emits the full direct-link native install guide with checksum commands and the stable Pear fallback. `npm run -s generate:native-install-smoke-plan -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --source-ref <merged-main-commit>` emits clean-host install smoke commands, a raw GitHub runtime RPC smoke helper, and evidence bullets from those resolver choices. `npm run generate:package-manager-manifests -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop` now prepares Homebrew Cask and WinGet draft manifests from public-trust assets by default, while `--trust-mode package-proof` rehearses the output shape against current package-proof assets. `npm run -s generate:native-signing-secret-plan -- --repo bigdestiny2/pearbrowser-desktop --tag v0.5.0 --source-ref <merged-main-commit>` emits the required and optional public-trust GitHub Actions secret inventory, guarded `gh secret set` command templates, and follow-up signing/readiness checks before a credential holder dispatches public-trust CI. `npm run check:native-signing -- --require-public-trust --secret-source github --repo bigdestiny2/pearbrowser-desktop` checks the required GitHub Actions secret names before dispatching a public-trust workflow, while warning that GitHub secret values remain unreadable until CI imports/signs/notarizes. `npm run check:public-trust-readiness -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --source-ref <merged-main-commit> --signing-secret-source github` now aggregates signing, published asset, byte-download, Linux metadata, smoke-plan, package-manager, and evidence-log blockers while pinning the nested runtime smoke helper source. `npm run -s generate:public-trust-operator-report -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --source-ref <merged-main-commit> --signing-secret-source github` formats that readiness state into a Markdown handoff with gate summaries, grouped blockers, deferrals, and next commands, including the release evidence handoff. `npm run -s generate:release-evidence-handoff` formats the operator evidence log into grouped manual rows with expected outcomes and copy-ready PASS/DEFER templates before final smoke evidence is filled.
...
One earlier desktop `npm test` run reported `401/402`; the immediately repeated compact full run passed `402/402`. During the native release asset checker update, one full-suite run also exposed the existing `nostr-index-room` verify-and-drop flake, while the isolated test and immediate full rerun passed. During the native install snippet generator update, the first full-suite run reported `456/457`, then the immediate full rerun passed `457/457`. During the native install smoke plan generator update, the first full-suite run reported `461/462` with truncated TAP output, then the immediate full rerun passed `462/462`. A later PR #49 CI run exposed a rare `nostr-bind` tamper-test flake where replacing a signature byte with `00` could leave an already-`00` byte unchanged; the tamper helper now flips the last byte deterministically, and `test/nostr-bind.test.js` passed `200/200` fresh-process repetitions before the full suite passed. The current release branch now passes `478/478` after guarded signing secret setup templates, release evidence final-decision blocker normalization, public-trust operator report evidence-handoff command coverage, the release evidence handoff generator, Nostr signature tamper-test hardening, public-trust operator report generator, native signing secret plan generator, direct-link native install guide generator, GitHub Actions signing secret-name preflight, Peercord launch-mode, peerit, catalogue-dedupe, naming-target, runtime-smoke, release-doc, release-evidence checker, live-catalogue provenance, announcement-decision fail-closed coverage, startup landing regression coverage, in-page `hyper://` link bridge coverage, native signing preflight, native release public-trust workflow mode, dual-architecture macOS native packaging, native download verifier, Linux AppImage metadata checker, package-manager license metadata, native install snippet generator, native install smoke plan generator, package-manager manifest draft generator, public-trust readiness aggregator, public-trust macOS DMG gate, empty-checkout pin guard, runtime storage gate, release story smoke coverage, native release asset checker/resolver coverage, and HiveRelay registry standalone source-install coverage landed.
...
- Replaced the desktop source-install dependency on a sibling HiveRelay checkout with npm `^0.20.2` dependencies for `p2p-hiverelay`, `p2p-hiverelay-client`, and `p2p-hiverelay-verifier`, plus a preinstall guard that keeps the lockfile on `0.20.2` and treats the sibling checkout as optional unless `file:` workspace deps are explicitly used.
...
- Added a native signing secret plan generator so a credential-holding operator gets the canonical GitHub Actions secret names, `gh secret set` command templates, and follow-up public-trust readiness checks without copying workflow YAML by hand.
- Guarded the native signing secret setup commands so generated `gh secret set` templates check for non-empty certificate files and env vars before uploading secrets.
- Added a public-trust operator report generator that turns the readiness result into a Markdown/JSON handoff with gate summaries, grouped blockers, warnings, and next commands for credentials, CI dispatch, clean-install smoke, package-manager drafts, and evidence.
- Added a release evidence handoff generator that turns the operator evidence log into grouped manual rows with expected outcomes and copy-ready PASS/DEFER templates before final smoke evidence is filled.
- Normalized the release evidence final-decision blocker so a blank final announcement decision is reported once with a combined reason, and added the release evidence handoff command to the public-trust operator report next steps.
...
- Added Linux AppImage metadata source plus a checker that validates the Linux icon, AppStream metainfo, generated desktop entry, icon, and metainfo in AppDirs/AppImages; the native release workflow now runs it on Linux before artifact collection.
- Added explicit root/package/AppStream license metadata (`Apache-2.0 AND MIT`) so package-manager manifest drafts no longer default WinGet license metadata to `Unknown`.
...
- Added a native install snippet generator so release notes and install pages can be generated from the actual attached package/checksum assets.
- Added a native clean-install smoke plan generator so macOS, Windows, and Linux clean-host smoke commands and evidence bullets are generated from the actual attached assets.
- Added a package-manager manifest draft generator for Homebrew Cask and WinGet, gated by public-trust assets by default.
- Added a public-trust readiness aggregator that runs the signing, published asset, byte-download, Linux AppImage metadata, clean-install smoke-plan, package-manager dry-run, and evidence-log gates and reports all announcement blockers together.
...
- `scripts/check-hiverelay-layout.mjs` confirms HiveRelay installs from npm (`^0.20.2`, lockfile `0.20.2`) for standalone source installs, exits quietly for registry deps, and validates the optional sibling HiveRelay checkout only when `file:` workspace deps are used.
...
- `scripts/generate-native-install-snippet.mjs` emits release-note/install-page Markdown for the recommended native packages and checksum sidecars from the attached GitHub release assets.
- `scripts/generate-native-install-smoke-plan.mjs` emits clean-host install smoke commands, a downloadable runtime RPC smoke helper URL from `--source-ref`, and evidence bullets for the recommended macOS, Windows, and Linux packages, refusing package-proof macOS zips in `--trust-mode public-trust`.
- `scripts/generate-package-manager-manifests.mjs` emits Homebrew Cask and WinGet singleton manifest drafts from attached GitHub release assets and checksum sidecars, refusing package-proof macOS assets unless explicitly run with `--trust-mode package-proof`.
- `scripts/check-public-trust-readiness.mjs` accepts `--source-ref` and forwards it to the clean-install smoke-plan subcheck so the top-level public-trust report pins the runtime RPC smoke helper used by clean hosts.
...
npm run check:linux-appimage-metadata -- --json # verified the committed Linux icon and AppStream metainfo source; Linux native release CI also checks the generated AppDir with --build-dir appling/build
npm run -s generate:native-install-snippet -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop # emitted release-note/install-page Markdown for the four recommended native packages and sidecars with package-proof trust warning
npm run -s generate:native-install-guide -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop # emitted full direct-link native install guide with checksum commands and stable Pear fallback
npm run -s generate:native-signing-secret-plan -- --repo bigdestiny2/pearbrowser-desktop --tag v0.5.0 --source-ref <merged-main-commit> # emits the public-trust GitHub Actions secret inventory, guarded gh secret set command templates, and follow-up signing/readiness checks for the credential-holding operator
npm run -s generate:native-install-smoke-plan -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --source-ref <merged-main-commit> --json # emits clean-host smoke commands, a raw GitHub runtime-rpc-smoke helper URL, and evidence bullets for the current package-proof macOS app.zips, Windows exe, and Linux AppImage; public-trust mode correctly failed because v0.5.0 has macOS app.zips, not notarized DMGs
npm run check:public-trust-readiness -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --source-ref <merged-main-commit> --signing-secret-source github --json # aggregates signing, public assets, native downloads, Linux metadata, smoke-plan, package-manager, and evidence blockers while pinning the nested runtime helper source
npm run -s generate:public-trust-operator-report -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --source-ref <merged-main-commit> --signing-secret-source github # formats the readiness state into a Markdown handoff with grouped blockers, deferrals, and next commands
npm run generate:package-manager-manifests -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --trust-mode package-proof --out-dir /private/tmp/pearbrowser-package-manager-drafts --json # rehearsed Homebrew Cask and WinGet draft generation against current package-proof assets; default public-trust dry-run correctly failed because v0.5.0 has macOS app.zips, not notarized DMGs
...
npm run check:native-signing -- --require-public-trust --secret-source github --repo bigdestiny2/pearbrowser-desktop # desktop public-trust signing preflight; checks GitHub Actions secret names and blocks until required macOS Developer ID/notary and Windows PFX secrets are configured
node scripts/check-hiverelay-layout.mjs # desktop source install guard; confirms HiveRelay installs from npm (^0.20.2 / lockfile 0.20.2), exits quietly without ../../00-core/hiverelay
npm ci --prefix /private/tmp/pear-standalone-source-install-29arBb # standalone source-install fixture with only package/lock/script; passes without ../../00-core/hiverelay
npm view p2p-hiverelay@0.20.2 version && npm view p2p-hiverelay-client@0.20.2 version && npm view p2p-hiverelay-verifier@0.20.2 version # confirms the expected HiveRelay release line is published to npm
