...
- Desktop: `npm test` passed `443/443` after the release story smoke tooling and evidence updates.
...
- Desktop native packaging is now wired through `.github/workflows/desktop-native-release.yml`: macOS emits an ad-hoc signed `.app.zip`, Windows emits `.msix` plus `.exe`, Linux emits `.AppImage`, and the collector attaches checksums plus manifests. The `v0.5.0` GitHub release asset backfill completed successfully again on 2026-06-28 in Desktop Native Release run `28317333414` from `main` with `source_ref=main` after the native signing credential preflight merge; all three platform jobs passed the shared credential checker, and the attach job refreshed 16 release assets around `2026-06-28T09:21Z`.
...
One earlier desktop `npm test` run reported `401/402`; the immediately repeated compact full run passed `402/402`, and the current release branch now passes `443/443` after the Peercord launch-mode, peerit, catalogue-dedupe, naming-target, runtime-smoke, release-doc, release-evidence checker, live-catalogue provenance, announcement-decision fail-closed coverage, startup landing regression coverage, in-page `hyper://` link bridge coverage, native signing preflight, empty-checkout pin guard, runtime storage gate, and release story smoke coverage landed. No code change was needed for that earlier blip.
...
gh workflow run desktop-native-release.yml --repo bigdestiny2/pearbrowser-desktop --ref main -f tag=v0.5.0 -f source_ref=main # desktop native release backfill, latest run 28317333414 succeeded after native signing credential preflight merge
gh release view v0.5.0 --repo bigdestiny2/pearbrowser-desktop --json tagName,isDraft,isPrerelease,publishedAt,assets # verified macOS, Windows, Linux native assets plus checksums/manifests attached
...
node scripts/release-rpc-story-smoke.mjs --timeout 45000 --request-timeout 30000 --json # after launching PearBrowser; latest live run passed: homepage HTTP 200/90770 bytes, 2 catalogues, 14 apps, Keet/PearPass/anonGPT/Paste/Peercord present, Peercord standalone/window-only; no third-party app launched
...
- Peercord cannot honestly be marketed as headless-in-tab until upstream ships a compatible pear-request worker. The historical live bundle contract confirmed the packaged app was desktop/window-class, but the current full-bundle availability gate is deferred until the Peercord drive is repinned from a complete source.
