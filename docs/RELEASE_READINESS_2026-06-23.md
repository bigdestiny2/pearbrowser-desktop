...
- `scripts/generate-native-install-smoke-plan.mjs` emits clean-host install smoke commands and evidence bullets for the recommended macOS, Windows, and Linux packages, refusing package-proof macOS zips in `--trust-mode public-trust`.
...
npm run -s generate:native-install-smoke-plan -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --json # emitted clean-host smoke commands and evidence bullets for the current package-proof macOS app.zips, Windows exe, and Linux AppImage; public-trust mode correctly failed because v0.5.0 has macOS app.zips, not notarized DMGs
