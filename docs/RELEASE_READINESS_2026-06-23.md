...
- Desktop: `npm test` passed `427/427`.
...
One earlier desktop `npm test` run reported `401/402`; the immediately repeated compact full run passed `402/402`, and the current release branch now passes `427/427` after the Peercord launch-mode, peerit, catalogue-dedupe, naming-target, runtime-smoke, release-doc, release-evidence checker, live-catalogue provenance, announcement-decision fail-closed coverage, and startup landing regression coverage landed. No code change was needed for that earlier blip.
