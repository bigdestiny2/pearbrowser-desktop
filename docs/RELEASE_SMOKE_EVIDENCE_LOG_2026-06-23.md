...
| Public-trust readiness aggregator | one command reports signing, public asset, download, Linux metadata, smoke-plan, package-manager, and evidence blockers | PASS | 2026-06-28 live run: `npm run -s check:public-trust-readiness -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --source-ref <merged-main-commit> --json` exits 1 as expected for the current package-proof release; current generator accepts `--source-ref` and forwards it to the clean-install smoke-plan subcheck so public-trust reports pin the downloaded runtime helper; it reported missing macOS Developer ID/notary credentials, missing Windows signing certificate, missing macOS arm64/x64 public-trust DMGs, public-trust smoke/manifest blockers for `.app.zip` macOS assets, `native-downloads` verified 4 targets, Linux metadata passed, package-manager license metadata no longer defaulted to `Unknown`, and release evidence still incomplete. Fixture tests cover the all-green public-trust shape and the package-proof blocker aggregation path |
