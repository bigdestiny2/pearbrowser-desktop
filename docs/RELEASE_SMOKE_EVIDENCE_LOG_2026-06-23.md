...
| --- | --- | --- | --- |
| `npm test` | 451/451 pass | PASS | 2026-06-28 local rerun after the dual-architecture macOS native packaging update: `node --test 'test/*.test.js'`, pass `451/451`; one earlier full-suite run showed the existing `nostr-index-room` verify-and-drop test flake, and the isolated test plus full rerun passed |
