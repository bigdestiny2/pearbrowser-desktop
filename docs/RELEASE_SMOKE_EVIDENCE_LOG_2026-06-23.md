...
| --- | --- | --- | --- |
| `npm test` | 468/468 pass | PASS | 2026-06-28 local rerun after the Linux AppImage metadata gate update: `node --test 'test/*.test.js'`, pass `468/468`; earlier native install smoke plan generator coverage saw a first full-suite run report `461/462` with truncated TAP output before the immediate rerun passed cleanly, and earlier release evidence also saw the existing `nostr-index-room` verify-and-drop flake pass on isolated and full reruns |
| `git diff --check` | clean | PASS | 2026-06-28 local run on `docs/native-install-guide` exited 0 after the native install guide and release evidence refresh |
