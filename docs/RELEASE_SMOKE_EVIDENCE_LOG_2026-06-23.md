...
| GitHub native release asset backfill | workflow visible on default branch; macOS, Windows, Linux assets attached to `v0.5.0` with sidecars/manifests | PASS | 2026-06-28 GitHub Actions run `https://github.com/bigdestiny2/pearbrowser-desktop/actions/runs/28316194210` succeeded from `main` with `tag=v0.5.0` and `source_ref=main` after the signing-gates workflow merge; release `v0.5.0` now has `PearBrowser-0.5.0-macos-arm64.app.zip`, Windows `.exe` and `.msix`, Linux `.AppImage` assets, per-asset `.sha256`, `SHA256SUMS-*`, and `manifest-*` files attached |
