...
| Native public-trust workflow mode | public release runs fail closed on missing signing/notary credentials | PASS | 2026-06-28 source-contract coverage verifies `.github/workflows/desktop-native-release.yml` exposes `release_mode` choices, defaults manual runs to `package-proof`, defaults release/tag-triggered runs to `public-trust`, adds `--require-public-trust` before `scripts/check-native-signing-credentials.mjs`, and adds `--require-published` before the post-upload native release asset check |
