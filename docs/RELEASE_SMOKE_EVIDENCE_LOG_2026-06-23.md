...
| --- | --- | --- | --- |
| `npm test` | 446/446 pass | PASS | 2026-06-28 local rerun after the native release asset checker update: `node --test 'test/*.test.js'`, pass `446/446`; one immediately preceding full-suite run showed the existing `nostr-index-room` verify-and-drop test flake, and the isolated test plus full rerun passed |
| `git diff --check` | clean | PASS | 2026-06-28 local run on `feat/release-site-story-smoke` exited 0 after the site-story smoke docs refresh |
