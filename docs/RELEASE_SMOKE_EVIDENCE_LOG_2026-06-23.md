...
| --- | --- | --- | --- |
| `npm test` | 453/453 pass | PASS | 2026-06-28 local rerun after the native download verifier update: `node --test 'test/*.test.js'`, pass `453/453`; one earlier full-suite run showed the existing `nostr-index-room` verify-and-drop test flake, and the isolated test plus full rerun passed |
