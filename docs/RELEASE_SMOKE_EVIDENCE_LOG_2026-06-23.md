...
| --- | --- | --- | --- |
| `npm test` | 473/473 pass | PASS | 2026-06-29 local rerun after Nostr signature tamper-test hardening and the public-trust operator report generator update: `node --test 'test/*.test.js'`, pass `473/473`; `test/nostr-bind.test.js` also passed `200/200` fresh-process repetitions after replacing no-op-prone signature mutations. Prior local rerun after the native signing secret plan generator update passed `471/471`; prior local rerun after the GitHub signing secret preflight update passed `470/470`; 2026-06-28 rerun after the package-manager license metadata update passed `469/469`; earlier native install smoke plan generator coverage saw a first full-suite run report `461/462` with truncated TAP output before the immediate rerun passed cleanly, and earlier release evidence also saw the existing `nostr-index-room` verify-and-drop flake pass on isolated and full reruns |
...
| Native public-trust workflow mode | public release runs fail closed on missing signing/notary credentials | PASS | 2026-06-28 source-contract coverage verifies `.github/workflows/desktop-native-release.yml` exposes `release_mode` choices, defaults manual runs to `package-proof`, defaults release/tag-triggered runs to `public-trust`, adds `--require-public-trust` before `scripts/check-native-signing-credentials.mjs`, creates a public-trust macOS DMG before artifact collection, adds `--require-published --require-public-trust` before the post-upload native release asset check, and runs `scripts/verify-native-downloads.mjs --all` after upload for public-trust releases |
