...
| --- | --- | --- | --- |
| `npm test` | 455/455 pass | PASS | 2026-06-28 local rerun after the public-trust macOS DMG gate update: `node --test 'test/*.test.js'`, pass `455/455`; one earlier full-suite run showed the existing `nostr-index-room` verify-and-drop test flake, and the isolated test plus full rerun passed |
