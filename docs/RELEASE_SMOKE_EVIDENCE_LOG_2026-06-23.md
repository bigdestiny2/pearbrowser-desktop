...
| --- | --- | --- | --- |
| `npm test` | 457/457 pass | PASS | 2026-06-28 local rerun after the native install snippet generator update: `node --test 'test/*.test.js'`, pass `457/457`; the immediately previous full-suite run reported one failure with truncated TAP output, and the immediate rerun passed cleanly. Earlier release evidence also saw the existing `nostr-index-room` verify-and-drop flake pass on isolated and full reruns |
