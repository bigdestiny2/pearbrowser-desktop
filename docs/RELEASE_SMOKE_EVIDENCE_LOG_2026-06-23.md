...
| --- | --- | --- | --- |
| `npm test` | 451/451 pass | PASS | 2026-06-28 local rerun after the native release public-trust workflow mode update: `node --test 'test/*.test.js'`, pass `451/451`; one earlier full-suite run showed the existing `nostr-index-room` verify-and-drop test flake, and the isolated test plus full rerun passed |
| `git diff --check` | clean | PASS | 2026-06-28 local run on `feat/native-release-public-trust-mode` exited 0 after the native release public-trust workflow mode update |
...
| Native asset resolver | recommended package and checksum sidecar resolve per platform | PASS | 2026-06-28 live release queries with `npm run resolve:native-release -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop`: macOS arm64 resolved `PearBrowser-0.5.0-macos-arm64.app.zip` plus `.sha256`, Windows x64 resolved `PearBrowser-0.5.0-windows-x64.exe` plus `.sha256`, Linux x64 resolved the normalized `PearBrowser-0.5.0-linux-x64.AppImage` plus `.sha256` even with a longer duplicate AppImage candidate present |
