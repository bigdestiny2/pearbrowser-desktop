...
| --- | --- | --- | --- |
| `npm test` | 462/462 pass | PASS | 2026-06-28 local rerun after the native install smoke plan generator update: `node --test 'test/*.test.js'`, pass `462/462`; the immediately previous full-suite run reported `461/462` with truncated TAP output, and the immediate rerun passed cleanly. The earlier package-manager manifest draft generator rerun passed `460/460`, and earlier release evidence also saw the existing `nostr-index-room` verify-and-drop flake pass on isolated and full reruns |
