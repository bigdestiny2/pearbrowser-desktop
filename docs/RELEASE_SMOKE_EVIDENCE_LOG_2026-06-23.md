...
| Native install smoke plan generator | clean-host install commands generated from live assets; public-trust gate refuses unsigned macOS zips | PASS | 2026-06-28 live run: `npm run -s generate:native-install-smoke-plan -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --json` emitted macOS, Windows, and Linux clean-host download/checksum/install/launch commands plus evidence bullets for the current package-proof assets; `--trust-mode public-trust` correctly failed with `public-trust clean-install smoke requires notarized macOS DMG assets` for both macOS architectures |
