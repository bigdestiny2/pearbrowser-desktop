...
| --- | --- | --- | --- |
| `npm test` | 460/460 pass | PASS | 2026-06-28 local rerun after the package-manager manifest draft generator update: `node --test 'test/*.test.js'`, pass `460/460`; the earlier native install snippet generator rerun passed `457/457` after one truncated failure, and earlier release evidence also saw the existing `nostr-index-room` verify-and-drop flake pass on isolated and full reruns |
