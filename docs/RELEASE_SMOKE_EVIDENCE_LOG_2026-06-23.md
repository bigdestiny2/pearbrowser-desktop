...
| Public-trust readiness aggregator | one command reports signing, public asset, download, smoke-plan, package-manager, and evidence blockers | PASS | 2026-06-28 live run: `npm run -s check:public-trust-readiness -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop --json` exited 1 as expected for the current package-proof release; it reported missing macOS Developer ID/notary credentials, missing Windows signing certificate, missing macOS arm64/x64 public-trust DMGs, public-trust smoke/manifest blockers for `.app.zip` macOS assets, `native-downloads` verified 4 targets, and release evidence still incomplete. Fixture tests cover the all-green public-trust shape and the package-proof blocker aggregation path |
