...
| --- | --- | --- | --- |
| `npm test` | 450/450 pass | PASS | 2026-06-28 local rerun after the native release asset resolver update: `node --test 'test/*.test.js'`, pass `450/450`; one earlier full-suite run showed the existing `nostr-index-room` verify-and-drop test flake, and the isolated test plus full rerun passed |
