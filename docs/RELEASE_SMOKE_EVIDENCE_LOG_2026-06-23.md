...
| Native install snippet generator | release-note install block generated from live assets | PASS | 2026-06-28 live run: `npm run -s generate:native-install-snippet -- --tag v0.5.0 --repo bigdestiny2/pearbrowser-desktop`; emitted Markdown links for macOS arm64/x64 `.app.zip`, Windows x64 `.exe`, Linux x64 `.AppImage`, each matching `.sha256` sidecar, plus package-proof install notes and trust warning |
