...
- The mobile Browse and catalogue changes pass tests, `ExpoLinking` is now present, generated Expo iOS Debug builds, the tracked SwiftUI iOS shell launches to `Connected`, Android native debug APK assembly plus emulator launch pass with Eclipse Temurin 17, and Android release APK/AAB plus disposable-key signing verification pass. Generated Expo iOS Release/Hermes cleanup, production Android keystore/store validation, broader real-device validation, and distribution checks are still required before app-store-style release.
