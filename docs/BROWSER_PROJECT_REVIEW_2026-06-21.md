...
Original 2026-06-21 limitation: live Pear GUI launch, live DHT/HiveRelay replication, real default Hyperbee availability from the public network, and mobile simulator/device runtime were not exercised in that first pass. The 2026-06-23 release pass closed the network/catalogue gaps: the live PearBrowser Network Hyperbee, production browser drive, Peercord bundle drive, and Keet bundle drive were fresh-peer verified with real network access. The mobile pass also fixed missing `ExpoLinking`, added stale-Corestore recovery, proved the tracked SwiftUI iOS shell reaches green `Connected` on simulator, and proved Android native Gradle task discovery plus Kotlin compile with OpenJDK 17. Remaining manual/runtime gaps are Pear's trust prompt for actually executing Peercord, generated Expo iOS cleanup if that shell remains release-targeted, Android APK assembly/device launch, and signing/distribution checks.
...
- The mobile Browse and catalogue changes pass tests, `ExpoLinking` is now present, the tracked SwiftUI iOS shell launches to `Connected`, and Android native Kotlin compile passes with OpenJDK 17. A clean generated Expo iOS pass, Android APK/device smoke, and distribution signing pass are still required before app-store-style release.
