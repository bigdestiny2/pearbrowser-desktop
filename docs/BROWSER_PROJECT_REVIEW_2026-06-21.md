...
- The mobile Browse and catalogue changes pass tests, `ExpoLinking` is now present, the tracked SwiftUI iOS shell launches to `Connected`, and Android native debug APK assembly plus emulator launch now pass with Eclipse Temurin 17. A clean generated Expo iOS pass, signed Android release APK/AAB artifacts, broader real-device validation, and distribution checks are still required before app-store-style release.
