...
- Production browser release availability is now proven by `node scripts/verify-pin.js --expect 18614 --hiverelay`: drive length `18614`, `/backend/anongpt-buyer.js` blob sampled, peers found, and upgraded HiveRelay relays can add `proveSeeded` evidence. `node scripts/verify-release-contents.js --expect 18614 --missing /.landing-seed.mjs --missing /pearbrowser-storage --missing /docs --missing /scripts --missing /examples --missing /test` also confirmed those operator-only paths are absent from a fresh peer.
