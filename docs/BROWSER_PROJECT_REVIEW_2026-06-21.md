...
- Production browser release availability is now proven by `node scripts/verify-pin.js --expect 18552 --hiverelay`: drive length `18552`, `/backend/anongpt-buyer.js` blob sampled, peers found, and upgraded HiveRelay relays can add `proveSeeded` evidence. `node scripts/verify-release-contents.js --expect 18552 --missing /.landing-seed.mjs --missing /pearbrowser-storage --missing /docs --missing /scripts --missing /examples --missing /test` also scanned 10,199 entries from a fresh peer and confirmed those operator-only paths are absent.
- Peercord bundle availability was proven on 2026-06-23, but current 2026-06-28 release evidence defers it: `node scripts/verify-app-full.js --key a2ea4d769d5e2b90caca4fbcb7f4b7b43caf43f2555b81201d3463ef89b55c26 --name peercord --samples 12 --timeout 90` now resolves peers but no file entries after the failed empty-checkout repin/cleanup attempt. Treat Peercord as catalogue-correct but not bundle-availability-cleared until a complete source is available and the fresh-peer sampler passes again.
