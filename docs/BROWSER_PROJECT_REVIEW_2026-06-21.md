...
- The mobile Browse and catalogue changes pass tests, and `ExpoLinking` is now present after a simulator launch exposed the missing native module. A clean simulator/device smoke is still required because WebView bridge timing is lifecycle-sensitive and local iOS generated native tooling is still hanging before a fresh relaunch.
