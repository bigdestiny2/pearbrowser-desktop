...
Original 2026-06-21 limitation: live Pear GUI launch, live DHT/HiveRelay replication, real default Hyperbee availability from the public network, and mobile simulator/device runtime were not exercised in that first pass. The 2026-06-23 release pass closed the network/catalogue gaps at the time: the live PearBrowser Network Hyperbee, production browser drive, Peercord bundle drive, and Keet bundle drive were fresh-peer verified with real network access. Current 2026-06-28 evidence keeps the catalogue, production browser drive, homepage, and Keet green, but Peercord bundle availability is deferred again because the fresh-peer refresh resolves an empty checkout. The mobile pass also fixed missing `ExpoLinking`, added stale-Corestore recovery, proved the tracked SwiftUI iOS shell reaches green `Connected` on simulator, proved generated Expo iOS Debug builds with `ExpoLinking` autolinked, and proved Android native debug APK assembly plus emulator launch to a green `Connected` Home screen with a verified JDK 17. Remaining manual/runtime gaps are Pear's trust prompt for actually executing Peercord, Peercord full-bundle reseed by the publisher/operator, signed Android release APK/AAB distribution checks, broader real-device validation, and app-store-style distribution checks.
...
2026-06-23 release result: `npm test` passed `415/415`, including the catalogue, Peercord, peerit, search, naming, Nostr, sync, and command-mirror additions that landed after this review.
...
- Live default-catalog availability is now proven by `node scripts/verify-live-catalog.js --expect-app peercord --expect-app peerit --expect-app hiveworm`: signed Hyperbee metadata present, catalogue length `273`, 14 apps, peers found, Peercord/peerit/HiveWorm rows present, Peercord carries `type: "standalone"` plus the upstream source/licence metadata, and peerit carries `type: "hypersite"`.
- Production browser release availability is now proven by `node scripts/verify-pin.js --expect 18640 --hiverelay`: drive length `18640`, `/backend/anongpt-buyer.js` blob sampled, peers found, and upgraded HiveRelay relays can add `proveSeeded` evidence. `node scripts/verify-release-contents.js --expect 18640 --missing /.landing-seed.mjs --missing /pearbrowser-storage --missing /docs --missing /scripts --missing /examples --missing /test` also scanned 10,233 entries from a fresh peer and confirmed those operator-only paths are absent.
- Peercord bundle availability was proven on 2026-06-23, but current 2026-06-28 release evidence defers it: `node scripts/verify-app-full.js --key a2ea4d769d5e2b90caca4fbcb7f4b7b43caf43f2555b81201d3463ef89b55c26 --name peercord --samples 12 --timeout 90` now resolves peers but no file entries after the failed empty-checkout repin/cleanup attempt. The upstream source is reachable at commit `ea260a3bfba279769acfbfe0a436140c87a0fa15` and still matches the desktop/window contract, but canonical bundle repair requires Peercord publisher/operator reseed or complete publisher storage. Treat Peercord as catalogue-correct but not bundle-availability-cleared until the fresh-peer sampler passes again.
