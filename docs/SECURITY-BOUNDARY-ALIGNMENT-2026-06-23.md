...
| HiveRelay HTTP fast-path | Relay URLs are limited to HTTP(S), transport defaults are scheme-aware, content responses are byte-capped, control responses are separately byte-capped, and hanging relay requests are actively destroyed on timeout | `backend/relay-client.js:10`, `backend/relay-client.js:16`, `backend/relay-client.js:39`, `backend/relay-client.js:275`, `backend/relay-client.js:322` | `test/relay-client-http.test.js`, `test/release-packaging.test.js` |
...
| `swarm.v1` page API | Topic joins are drive-derived Tier A, persisted-grant Tier B, or consent-gated Tier C; logical channels are Protomux sub-channels keyed by `(protocol, topic)`; per-app channel/join limits, one pending consent, per-peer outbound byte cap | `backend/swarm-bridge.js:14`, `backend/swarm-bridge.js:33`, `backend/swarm-bridge.js:108`, `backend/swarm-bridge.js:312`, `backend/swarm-bridge.js:380`, `backend/swarm-bridge.js:501` | `test/swarm-v1-protomux.test.js`, `test/mobile-source-contract.test.js`, `docs/SWARM-V1.md`, `docs/PEARBROWSER-APP-COMPAT-STANDARD.md` |
