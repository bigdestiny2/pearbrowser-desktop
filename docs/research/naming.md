...
> **Current implementation note (2026-06-22):** the core naming layer described
> here has since landed in the desktop branch: local petnames, owned registry
> records, trusted-contact federation, curated aliases, `pearname://`, and
> provenance-aware resolution. Treat sections that say "no name-resolution
> layer" as historical motivation, not current product state. See
