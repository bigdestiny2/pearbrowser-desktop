...
// Records a legacy app identifier and returns non-executable migration guidance.
// This slot deliberately replaces the old remote Pear-link launcher.
const CMD_LEGACY_APP_MIGRATION = 28
...
const CMD_DEVICE_LINK_CREATE_INVITE = 76  // source device: mint a blind-pairing invite to link a new device
const CMD_DEVICE_LINK_JOIN = 77  // target device: join with an invite, receive + adopt the root seed
...
// WDK wallet preview (docs/WDK_WALLET_V0.9_SPEC.md §10) — browser-chrome /
// internal RPC only; Hyperdrive pages use the HTTP bridge contract (spec §4).
// 306–308 are implemented chrome-owned read commands (address, balances,
// journal-backed history). 313 is a reserved spec allocation: reconcile ships
// with a later phase — its handler fails closed with `not-implemented` until
// then.
const CMD_WALLET_STATUS = 300
const CMD_WALLET_CREATE = 301
const CMD_WALLET_IMPORT = 302
const CMD_WALLET_BACKUP = 303
const CMD_WALLET_UNLOCK = 304
const CMD_WALLET_LOCK = 305
const CMD_WALLET_ADDRESS = 306
const CMD_WALLET_BALANCES = 307
const CMD_WALLET_TRANSACTIONS = 308
const CMD_WALLET_CONNECTIONS_LIST = 309
const CMD_WALLET_CONNECTION_REVOKE = 310
const CMD_WALLET_CONNECT_RESOLVE = 311
const CMD_WALLET_PAYMENT_RESOLVE = 312
const CMD_WALLET_RECONCILE = 313

/** Wallet consent + lifecycle push events (spec §10). Payloads are always
 *  safe display records — never tokens, manifests or secrets. */
const EVT_WALLET_CONNECT_REQUEST = 112
const EVT_WALLET_PAYMENT_REQUEST = 113
const EVT_WALLET_TX_UPDATE = 114
...
module.exports = {
  ...
  CMD_UNPUBLISH_SITE, CMD_LIST_SITES, CMD_DELETE_SITE, CMD_LOAD_TEMPLATE, CMD_GET_SITE_BLOCKS, CMD_LEGACY_APP_MIGRATION, CMD_RESET_APP,
  ...
  CMD_PRIVACY_STATUS,
  CMD_WALLET_STATUS, CMD_WALLET_CREATE, CMD_WALLET_IMPORT, CMD_WALLET_BACKUP,
  CMD_WALLET_UNLOCK, CMD_WALLET_LOCK,
  CMD_WALLET_ADDRESS, CMD_WALLET_BALANCES, CMD_WALLET_TRANSACTIONS,
  CMD_WALLET_CONNECTIONS_LIST, CMD_WALLET_CONNECTION_REVOKE,
  CMD_WALLET_CONNECT_RESOLVE, CMD_WALLET_PAYMENT_RESOLVE, CMD_WALLET_RECONCILE,
  CMD_BRIDGE,
  CMD_STOP,
  ...
  EVT_WALLET_CONNECT_REQUEST, EVT_WALLET_PAYMENT_REQUEST, EVT_WALLET_TX_UPDATE,
  ANONGPT_DRIVE_KEY,
}
