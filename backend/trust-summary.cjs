...
function targetDriveKey (record, explicitDriveKey) {
  const r = asObject(record)
  return normalizeDriveKey(explicitDriveKey) || normalizeDriveKey(r.driveKey || r.keyHex)
}
...
function verificationBadge (verification) {
  if (verification === 'author-signed') return badge('verification:author-signed', 'author-signed', 'followed', 'Catalog row is signed by the app author')
  if (verification === 'relay-listed') return badge('verification:relay-listed', 'relay-listed', 'followed', 'Catalog row is listed by a relay index')
...
function publisherBadges (record) {
  ...
  if (p.name) return [badge('publisher:name', `by ${p.name}`, verification === 'author-signed' ? 'followed' : 'self', p.key ? `Publisher key ${shortKey(p.key)}` : 'Publisher identity metadata')]
...
function appLaunchBadge (app) {
  ...
  if (a.type === 'hypersite') return badge('launch:tab', 'tab app', 'self', 'Runs inside a browser tab')
  if (/^pear:\/\//i.test(link)) return badge('launch:window', 'window app', 'other', 'Opens in its own Pear window')
  if (/^(?:hyper|file):\/\//i.test(link) || compactLabel(a.driveKey, 80)) return badge('launch:site', 'hyper site', 'self', 'Opens as peer-served site content in a tab')
...
function releaseEntries (app) {
  ...
  const push = (entry) => {
    ...
    const date = dateLabel(r.updatedAt || r.publishedAt || r.releasedAt)
...
function pinSummary (pin) {
  ...
  const accepted = finiteCount(p.acceptances) ?? finiteCount(p.seedAcceptances)
  const peers = finiteCount(p.replicatedPeers) ?? finiteCount(p.activePeers)
  if (state === 'relay-confirmed') return `Relay pin confirmed${peers !== null ? ` with ${peers} replicated ${peers === 1 ? 'peer' : 'peers'}` : ''}`
  if (state === 'seeded') return `Relay pin accepted${accepted !== null ? ` by ${accepted} ${accepted === 1 ? 'relay' : 'relays'}` : ''}`
...
function availabilityBadges (driveInfo) {
  ...
  if (bytesLabel) out.push(badge('availability:size', bytesLabel, 'self', 'Local known bundle size'))
  return out
}
...
function availabilitySummary (driveInfo) {
  const info = asObject(driveInfo)
  const peers = finiteCount(info.peerCount)
  const bytesLabel = formatBytes(finiteCount(info.byteLength))
  const parts = []
  if (peers !== null) parts.push(`${peers} ${peers === 1 ? 'peer' : 'peers'} serving`)
  if (bytesLabel) parts.push(bytesLabel)
  return parts.join(', ')
}
...
function permissionBadges (evidence) {
  ...
  if (Array.isArray(p.profileFields) && p.profileFields.length) out.push(badge('permission:profile', `${p.profileFields.length} profile`, 'other', `Shared profile fields: ${listWords(p.profileFields, 6)}`))
  if (p.contactAccess) out.push(badge('permission:contacts', 'contacts', 'other', 'This app can read your saved contacts'))
  if (Array.isArray(p.swarm) && p.swarm.length) out.push(badge('permission:swarm', `${p.swarm.length} swarm`, 'other', `${p.swarm.length} persistent arbitrary-topic swarm ${p.swarm.length === 1 ? 'grant' : 'grants'}`))
  return out
}
...
function permissionSummary (evidence) {
  ...
  if (Array.isArray(p.swarm) && p.swarm.length) parts.push(`Has ${p.swarm.length} persistent swarm topic ${p.swarm.length === 1 ? 'grant' : 'grants'}`)
  return parts
}

function appTrustSummary (app, { driveInfo, loginGrants, swarmGrants, driveKey } = {}) {
  const a = asObject(app)
  ...
  const releases = releaseEntries(a)
  const badges = [verificationBadge(a.verification), ...publisherBadges(a), appLaunchBadge(a)]
  if (sources.length > 1) badges.push(badge('catalog:sources', `${sources.length} catalogs`, 'self', `Listed by ${sources.join(', ')}`))
  else if (sources.length === 1) badges.push(badge('catalog:source', compactLabel(sources[0], 32), 'self', `Listed by ${sources[0]}`))
  ...
  const parts = [verificationSummary(a.verification), publisherSummary(a), appLaunchSummary(a)].filter(Boolean)
  if (sources.length > 1) parts.push(`Listed by ${sources.length} catalogs`)
  else if (sources.length === 1) parts.push(`Listed by ${sources[0]}`)
  const releaseText = releaseSummary(a)
  if (releaseText) parts.push(releaseText)
  ...
  return {
    ...
    summary: parts.join('. '),
    evidence: {
      driveKey: targetKey || null,
      publisher: publisherIdentity(a),
      catalogSources: sources,
      releaseCount: releases.length,
      signedReleaseCount: releases.filter((release) => release.signed).length,
      permissionCount: (permissions.login ? 1 : 0) + (Array.isArray(permissions.swarm) ? permissions.swarm.length : 0)
    }
  }
}

function siteTrustSummary (site, { owned = false, loginGrants, swarmGrants, driveKey } = {}) {
  const s = asObject(site)
  const published = !!(s.published || s.driveKey || s.keyHex)
  const key = targetDriveKey(s, driveKey)
  const permissions = permissionEvidence({ driveKey: key, loginGrants, swarmGrants })
...
function buildTrustSummary ({ kind = 'app', app, site, driveInfo, loginGrants, swarmGrants, driveKey, owned } = {}) {
  return kind === 'site'
    ? siteTrustSummary(site || app, { owned, loginGrants, swarmGrants, driveKey })
    : appTrustSummary(app || site, { driveInfo, loginGrants, swarmGrants, driveKey })
}
