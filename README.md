...
The desktop currently consumes HiveRelay as local workspace packages at `../../00-core/hiverelay/packages/{core,client,verifier}`. Those `0.20.2` packages are not published to npm yet, so the sibling checkout is required for community source installs until the relay packages are published.
...
| `scripts/check-relays.js --require-relay --json` | Discovery probe - print all HiveRelays reachable via DHT, emit a `RESULT:` payload, and fail when no relay is reachable. |
...
| `npm run evidence:desktop -- --write --ci-url <Desktop CI run URL>` | Runs the safe automated desktop evidence gates and patches the matching rows in the release evidence log. |
| `scripts/verify-app-full.js <target>` | Deeper fresh-peer blob sampling across a drive's file tree, with release presets for `homepage`, `peercord`, and `keet`. |
| `scripts/verify-pear-bundle-contract.js <target>` | Metadata-only Peercord bundle contract check for `peercord-linux` or `peercord-windows`: reads `pear.json` and selected files from a fresh peer without executing third-party code. |
...
| [`bigdestiny2/P2P-Hiverelay`](https://github.com/bigdestiny2/P2P-Hiverelay) | The always-on relay backbone keeping the whole network alive; this desktop checkout currently consumes the local `0.20.2` workspace packages and verifies live relay compatibility through capability docs. |
