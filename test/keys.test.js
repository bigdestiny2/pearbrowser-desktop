...
test('normalizeUrl canonicalizes URL-bar input', () => {
  ...
  assert.equal(normalizeUrl('pear://abc'), null)
  assert.equal(normalizeUrl('file:///tmp/app'), null)
...
test('normalizeNameTarget accepts drive keys and browsable Hyper links only', () => {
  ...
  assert.equal(normalizeNameTarget('pear://keet'), null)
  assert.equal(normalizeNameTarget('file:///tmp/app'), null)
