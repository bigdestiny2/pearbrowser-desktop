...
test('caps total verifies at the per-query budget and flags exhaustion', () => {
  const planner = mkPlanner()
  ...
  assert.equal(sources[0].candidates.length, 256)      // bounded
})

test('the verify budget is shared across peers (a second peer is starved after the first exhausts it)', () => {
  const planner = mkPlanner()
  ...
  assert.equal(planner._verifyBudgetExhausted, true)
  // p1 consumed the budget; the loop breaks before p2 contributes
  assert.equal(sources.length, 1)
  assert.equal(sources[0].rootPubkey, p1.rootPubkey)
})
