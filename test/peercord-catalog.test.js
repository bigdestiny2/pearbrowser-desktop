...
test('Featured Peercord card uses the window launch path until a headless worker exists', () => {
  ...
  const entry = apps.match(/id: 'peercord'[\s\S]*?gradient: 'linear-gradient\([^']+\)'/)
  assert.ok(entry, 'Peercord featured entry missing')
  assert.match(entry[0], /type: 'standalone'/)
  assert.match(apps, new RegExp(`const PEERCORD_LINK = '${PEERCORD_LINK.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}'`))
  assert.match(entry[0], /link: PEERCORD_LINK/)
  assert.match(apps, /catalogue type is/)
})

test('Peercord launch shows a first-run standalone trust warning', () => {
  const apps = readFileSync(new URL('../ui/components/apps.js', import.meta.url), 'utf8')
  assert.match(apps, /STANDALONE_PRELAUNCH_WARNINGS/)
  assert.match(apps, /persistent third-party trust prompt/)
  assert.match(apps, /Approving it executes third-party code and creates a persistent trust decision/)
  assert.match(apps, /standaloneLaunchWarningsSeen/)
  assert.match(apps, /pendingStandaloneLaunch/)
  assert.match(apps, /app\.type === 'hypersite'/)
})
